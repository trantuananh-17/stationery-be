...
async function bootstrap() {
  ...
  Logger.debug(
    'GRPC CONFIG',
    JSON.stringify(configService.get('GRPC_SERV.GRPC_NOTIFICATION_SERVICE')),
  );

  const grpcPackage = configService.get<string>('GRPC_SERV.GRPC_NOTIFICATION_SERVICE.name');
  const grpcProtoPath = configService.get<string>(
    'GRPC_SERV.GRPC_NOTIFICATION_SERVICE.options.protoPath',
  );
  const grpcUrl = configService.get<string>('GRPC_SERV.GRPC_NOTIFICATION_SERVICE.options.url');

  if (!grpcPackage || !grpcProtoPath) {
    throw new Error('Missing GRPC_NOTIFICATION_SERVICE config');
  }
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      ...
      consumer: {
        groupId: QUEUE_GROUPS.NOTIFICATION,
