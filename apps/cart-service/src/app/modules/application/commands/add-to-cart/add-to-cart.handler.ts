  ...
  async execute(command: AddToCartCommand): Promise<void> {
    ...
    return await this.dataContext.runInTransaction(async () => {
      ...
      if (existedItem) {
        ...
        const finalQuantity = Math.min(nextQuantity, productCartItem.stock);

        existedItem.changeQuantity(finalQuantity);
        ...
        cart.addItem({
          ...
          quantity: Math.min(quantity, productCartItem.stock),
